...
export function CardView({
  ...
  const cardWidthHeight = compact ? 'w-40 h-56 text-[10px]' : 'w-56 h-80 text-sm';
    ...
    <div
      ...
      className={`${cardWidthHeight} bg-[#2e1065] border-2 ${
        isHoldReady
          ...
          : canManifest
          ? 'border-cyan-400 hover:border-cyan-300 shadow-[0_0_20px_rgba(34,211,238,0.3)] hover:-translate-y-6 cursor-pointer'
          : 'border-slate-700 opacity-80 cursor-not-allowed hover:opacity-100'
      } rounded-xl relative shadow-2xl transition-all flex flex-col select-none group ${className}`}
    >
      {/* Cost Badge */}
      ...
      {/* Card Content */}
      <div className="p-3 pt-4 flex flex-col h-full justify-between">
        {/* Title */}
        ...
        <div className={`${compact ? 'h-20' : 'h-28'} my-1 bg-indigo-950/80 rounded-lg flex items-center justify-center relative overflow-hidden border border-indigo-800/50`}>
          ...
          <GhostIcon artKey={card.artKey} className={`${compact ? 'w-16 h-16' : 'w-20 h-20'} z-10 transition-transform group-hover:scale-110`} />
        </div>

        {/* Keywords Row */}
        {card.keywords.length > 0 && (
          <div className="flex flex-wrap gap-1 justify-center my-0.5">
        ...
        {/* Text Area */}
        <div className="text-[8.5px] leading-tight space-y-1 bg-indigo-950/60 p-1.5 rounded flex-1 overflow-y-auto border border-indigo-900/40 text-slate-200">
          {card.manifestText && (
            <div>
